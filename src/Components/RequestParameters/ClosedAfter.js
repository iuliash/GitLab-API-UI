...
class ClosedAfter extends React.Component {
    constructor(){
        super();
    ...
    setDateTime(type, e) {
        ...
        request.closed_after[type] = e.target.value;
        ...
        return(
            <div className="form-item">
                ...
                <div className="date-time">
                    ...
                    <select className="date-time__item" defaultValue={'25'} onChange={e => {this.setDateTime('day', e)}}>
                        {days.map(day => (
                            <option value={day} key={day}>{day}</option>
                        ))}
                    </select>
                    <select className="date-time__item" defaultValue={'09'} onChange={e => {this.setDateTime('hour', e)}}>
                        {hours.map(hour => (
                            <option value={hour} key={hour}>{hour}</option>
                        ))}
                    </select>
                    <select className="date-time__item" defaultValue={'21'} onChange={e => {this.setDateTime('minute', e)}}>
                        {minutes.map(minute => (
...
export default connect(mapStateToProps, mapDispatchToProps)(ClosedAfter);